...
def data_reader(filename, num_pixel=784):
    ...
    labels = dataframe.ix[:, 'label'].values # read label
    to_keep = ((labels == 0) | (labels == 1) | (labels == 2) | (labels == 3) | (labels == 4) | (labels == 5) |(labels == 6) | (labels == 7) | (labels == 8) | (labels == 9))
    sub_dataframe = dataframe[to_keep]
    pixel_cols = ['pixel{0}'.format(i) for i in range(num_pixel)]
    images = sub_dataframe.ix[:, pixel_cols].values # read image pixel
...
class DataSet(object):
    def __init__(self, images, labels):
        ...
        # Converts the image value from [0, 255] to [0.0, 1.0].
