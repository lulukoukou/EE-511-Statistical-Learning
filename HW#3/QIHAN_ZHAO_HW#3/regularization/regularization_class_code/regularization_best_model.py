...
def fill_feed_dict(data_set, batch_size, image_ph, label_ph, keep_prob):
    ...
    feed_dict = {image_ph:x,label_ph:y, keep_prob:0.5}
    return feed_dict


def feed_forward_net(images, config, keep_prob):
...
def evaluation(sess, image_ph, label_ph, data_set, eval_op,keep_prob):
    ...
    epoch = (data_set.num_samples//batch_size)
    total = epoch * batch_size
    for step in range(epoch):
        feed_dict = fill_feed_dict(data_set,batch_size,image_ph,label_ph,keep_prob)
