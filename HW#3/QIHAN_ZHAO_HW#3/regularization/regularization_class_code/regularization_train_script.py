...
from regularization_best_model import *
from regularization_data import *
...
def train(config, train_data_set, valid_data_set=None):
    ...
    with tf.Graph().as_default():
        ...
        images_placeholder, labels_placeholder, keep_prob = placeholder_inputs_feedfoward(
            config.batch_size, config.image_pixel_size
        )

        # Builds feedforward with two hidden layers.
        logits = feed_forward_net(images_placeholder, config, keep_prob)
        ...
        with tf.Session() as sess:
            ...
            if valid_data_set:
                print('Validation data evlauation:')
                acc = evaluation(sess, images_placeholder, labels_placeholder,
                                 valid_data_set, corrent_cnt_op,keep_prob)
                print('valid accuracy: {:.3f}'.format(acc))
