...
# TODO: You can modify this to change the network.
        self.hidden1_size = 128
        self.hidden2_size = 128
# TODO: You need to change this for multi-class.
        self.num_class = 2
...
def feed_forward_net(images, config):
    ...
    """
# TODO: Creates the 1st feed fully-connected layer with ReLU activation.
    with tf.variable_scope('hidden_layer_1'):
        ...
        # Fill in your code below.
        hidden1_weights = tf.Variable(tf.random_normal([config.image_pixel_size, config.hidden1_size]))
        hidden1_biases = tf.Variable(tf.random_normal([config.hidden1_size]))
...
# TODO: Creates the 2nd feed fully-connected layer with ReLU activation.
    with tf.variable_scope('hidden_layer_2'):
        ...
        # Fill in your code below.
        hidden2_weights = tf.Variable(tf.random_normal([config.hidden1_size, config.hidden2_size]))
        hidden2_biases = tf.Variable(tf.random_normal([config.hidden2_size]))
...
# TODO: Creates the pen-ultimate linear layer.
    with tf.variable_scope('logits_layer'):
        ...
        # Fill in your code below.
        logits_weights = tf.Variable(tf.random_normal([config.hidden2_size, config.num_class]))
        logits_biases = tf.Variable(tf.random_normal([config.num_class]))
...
def compute_loss(logits, labels):
    ...
    Returns:
        loss: Loss tensor.
    """
    # TODO: Computes the cross-entropy loss.
...
def evaluation(sess, image_ph, label_ph, data_set, eval_op):
    ...
    x = data_set.images
    y = data_set.labels
    number = data_set.num_samples
    accuracy = sess.run(eval_op, feed_dict = {image_ph:x, label_ph:y})
