            ...
            for i in range(1):
                for j in range(step_per_epoch):
                    ...
                    for k in range(9):
                        ...
                        if k == 0:
                            valid_loss_, last = sess.run([self.loss,self.cell_final_state],
                                                                 feed_dict = {model.x_char: x_c, model.x_lang: x_l, model.y: y_l})
                        else:
                            valid_loss_, last = sess.run([self.loss,self.cell_final_state],
                                                                 feed_dict = {model.x_char: x_c, model.x_lang: x_l, model.y: y_l, self.cell_init_state : last})
                    ...
                    valid = valid_loss.index(min(valid_loss))
                    valid_id.append(language[valid])
