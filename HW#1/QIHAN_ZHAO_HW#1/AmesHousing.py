...
sale_price_test = []
slice = 1
for line in open('AmesHousing.txt'): # read each line from text file
    if flag:
        for item in line.split('\t'): # first line indicates features
        ...
        for item in line.split('\t'):
            ...
            index += 1
        table.append(temp)
        if slice%5==3:    # separate data set based on requirements
...
for f in feature:
    if (f not in numerical_variables) and (f not in discrete_variables):
        del df[f]                     # remove those features not required in given features
    else:
        feature_copy.append(f)
feature = feature_copy
# df includes data corresponding to features
# separate to training, validation and test set based on requirements
train = []
test = []
validation = []
...
def best_fit_slope_and_intercept(x,y): # function employs the one variable least squares equation
    m = (mean(x)*mean(y)-mean(x*y))/((mean(x)*mean(x))-(mean(x**2)))
    b = mean(y) - m*mean(x)
    return m,b

def rmse(predict, y): # rmse function to calculate root mean squared error
...
validation = []
test = []
df = pd.get_dummies(df, columns=discrete_variables, drop_first=True) # one-hot encoding by dummy variables
new_table = df.values.tolist()
for line in range(df.shape[0]): # after encoding, concatenate with numerical features and separate to train, validation and test sets
    index = int(table[line][0])
    if index % 5 == 3:
        validation.append(new_table[line])   
    elif index % 5 == 4:
...
reg = LinearRegression() # emply sklearn.linear_model.LinearRegression model for all variables least squares model
reg.fit(train,sale_price_train) # train the model
...
norm_train = (data_train - mean_train)/(std_train)  # normalize train, validation and test sets by substracting train mean and dividing by train std
...
for i in range(50,501,50):
    ...
    for j in range(k_folder):
        reg_lasso.fit(norm_train_validation[k:k+586],new_sale_price[k:k+586]) # cross-validaitn with 4 folders,
        if first:                                                             # train the model with same Alpha four times
            total_w = np.zeros(shape=reg_lasso.coef_.shape, dtype=np.float64) # then average the results
            first = False
    ...
    average_w.append(average)    # average_w stores each model's coefficients matrix

nonzero_w = []
Alpha = [x for x in range(50,501,50)]
for index in range(len(average_w)):
    nonzero_w.append(np.count_nonzero(average_w[index])) # count non-zero coefficients for each Alpha
...
reg_line_test = [(m*x)+b for x in Gr_Liv_Area_test] # apply one variable model to test set
RMSE_test = rmse(reg_line_test, sale_price_test)
print ("The RMSE(Gr_Liv_Area feature)for test is %s" %RMSE_test)

reg_predict_test = reg.predict(test)                # apply all variable model to test set
RMSE_test = rmse(reg_predict_test, sale_price_test)
print ("The RMSE(all features) for test is %s" %RMSE_test)

plt.figure(4) 
plt.xlabel('Alpha')                             # apply Lasso Regression model to test set
plt.ylabel('RMSE')
plt.title('RMSE for each Alpha')
