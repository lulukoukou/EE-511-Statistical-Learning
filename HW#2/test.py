...
class SGDTest(unittest.TestCase):
    ...
    def test_submission(self):
        ...
        predictions = [predict(model, point) for point in train_data]
        print("Training Accuracy: {0}".format(accuracy(train_data, predictions)))
        predictions = [predict(model, point) for point in valid_data]
        print("Validation Accuracy: {0}".format(accuracy(valid_data, predictions)))
